...
def test_auth_middleware_uses_constant_time_comparison():
    ...
    context = build_test_context(
        auth_enabled=True, auth_token="secret_token", read_only=False
    )
    ...
    with patch("kraken_bot.ui.api.secrets.compare_digest") as mock_digest:
        ...
        response = client.get(
            "/api/portfolio/summary", headers={"Authorization": "Bearer wrong_token"}
        )
        ...
        ), "Expected success when compare_digest is mocked to True"
