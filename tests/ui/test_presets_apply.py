...
def test_apply_preset_dry_run(test_client, tmp_path):
    ...
    ctx.config.profiles = {
        "test_profile": ProfileConfig(
            name="test_profile", config_path="profiles/test.yaml"
        )
    }
    ...
    preset_dir = config_dir / "presets" / "risk"
    preset_dir.mkdir(parents=True)
    (preset_dir / "Aggressive.yaml").write_text(
        yaml.safe_dump(
            {
                "name": "Aggressive",
                "kind": "risk",
...
def test_apply_preset_no_active_profile(test_client):
    ...
    response = test_client.post(
        "/api/presets/apply", json={"kind": "risk", "name": "Any", "dry_run": True}
    )
...
def test_apply_preset_invalid_payload(test_client, tmp_path):
    ...
    (preset_dir / "BadPayload.yaml").write_text(
        yaml.safe_dump(
...
def test_apply_preset_kind_mismatch(test_client, tmp_path):
    ...
    (preset_dir / "Mismatch.yaml").write_text(
        yaml.safe_dump({"name": "Mismatch", "kind": "strategies", "payload": {}})
    )

    response = test_client.post(
        "/api/presets/apply", json={"kind": "risk", "name": "Mismatch", "dry_run": True}
    )
