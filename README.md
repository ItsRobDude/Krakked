...
| **Current Product Track** | 🚧 In progress | Pinned-image Unraid deploy/upgrade/rollback proof, operator truth-labeling, and deterministic seeded emergency-flatten proof have passed; the next lane is decision-loop proof/soak acceptance, followed by paper/execution polish, strategy evidence reporting, ML/research reporting, and commercial/distribution polish. |
...
* A short decision-useful paper validation pass on `v0.1.1-rc.8` produced strategy intents, OMS dry-run fills, paper trades, and portfolio positions; the deterministic seeded emergency-flatten drill now covers cancel-first/retry/resume safety, and the next milestone is the decision-loop proof/soak acceptance lane in [`docs/decision-loop-proof-plan.md`](docs/decision-loop-proof-plan.md).
