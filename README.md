...
| **Current Product Track** | 🚧 In progress | Pinned-image Unraid deploy/upgrade/rollback proof, operator truth-labeling, deterministic seeded emergency-flatten tests, the `v0.1.1-rc.10` forward paper decision-loop soak, and the `v0.1.1-rc.11` controlled paper emergency-flatten confirmation have passed. The next lane is clamp/block diagnostic cleanup, validate-only drill criteria, paper/execution polish, strategy evidence reporting, ML/research reporting, and commercial/distribution polish. |
...
* A short decision-useful paper validation pass on `v0.1.1-rc.8` produced strategy intents, OMS dry-run fills, paper trades, and portfolio positions. The `v0.1.1-rc.10` forward decision soak then produced a longer paper decision stream with risk decisions, OMS fills, trades, snapshots, and legible score-filter diagnostics; see [`docs/soak-reports/2026-06-23-decision-soak-rc10-forward.md`](docs/soak-reports/2026-06-23-decision-soak-rc10-forward.md). That run exposed a paper emergency-flatten runtime defect, and the follow-up `v0.1.1-rc.11` confirmation proved the background paper emergency-flatten resume path now submits priced market closes, ingests paper trades, reduces the synthetic wallet, and clears emergency intent; see [`docs/soak-reports/2026-06-23-rc11-paper-flatten-confirmation.md`](docs/soak-reports/2026-06-23-rc11-paper-flatten-confirmation.md).
...
Use `replay-ready` when you want one command to refresh OHLC over public Kraken market-data endpoints and then print the offline replay preflight. Use `replay-run` when you want the guarded operator path: refresh, require clean readiness, run the replay, and publish the latest summary. Use the lower-level `backtest-preflight` and `backtest --publish-latest` commands when you need a purely cached/offline replay or custom report flow. Add `--fee-bps` to tune the flat taker-fee assumption, `--strict-data` to hard-fail on missing or partial coverage in the lower-level replay, and `--db-path backtest.db` if you want to keep the SQLite decisions/orders/results after the run. Use `poetry run krakked compare-backtests --baseline a.json --candidate b.json` to compare two saved reports without rerunning. See [`docs/simulation.md`](docs/simulation.md) for the current assumptions, limits, and replay smoke scenarios.
...
* `poetry run krakked refresh-ohlc --since <iso>` — refresh cached OHLC tails from public market-data endpoints before replay work.
* `poetry run krakked replay-ready --start <iso> --end <iso>` — network-refresh OHLC, then print the offline replay readiness check.
* `poetry run krakked replay-run --start <iso> --end <iso>` — network-refresh OHLC, require clean readiness, run the replay, and publish the latest replay summary.
...
* `force-link-submit-unknown --local-id <id> --kraken-id <txid> --reason <text>`: Operator-confirmed, audited link of a submit intent to a Kraken txid after manual verification. By default it **refuses** if the txid is not found in OpenOrders/ClosedOrders (fail closed); when found it imports the order's status/fills. Add `--allow-unverified-txid` only as a deliberate **break-glass** override — link to a txid the lookup cannot confirm (e.g., during Kraken/client lookup weirdness) — recorded in the audit trail as operator-accepted risk. It exists so DB surgery is never the only recovery path.
