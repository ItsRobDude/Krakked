...
| **Current Product Track** | 🚧 In progress | Pinned-image Unraid deploy/upgrade/rollback proof, operator truth-labeling, deterministic seeded emergency-flatten tests, and the `v0.1.1-rc.10` forward paper decision-loop soak have passed. The next lane is paper emergency-flatten runtime repair, clamp/block diagnostic cleanup, then paper/execution polish, strategy evidence reporting, ML/research reporting, and commercial/distribution polish. |
...
* A short decision-useful paper validation pass on `v0.1.1-rc.8` produced strategy intents, OMS dry-run fills, paper trades, and portfolio positions. The `v0.1.1-rc.10` forward decision soak then produced a longer paper decision stream with risk decisions, OMS fills, trades, snapshots, and legible score-filter diagnostics; see [`docs/soak-reports/2026-06-23-decision-soak-rc10-forward.md`](docs/soak-reports/2026-06-23-decision-soak-rc10-forward.md). The same run exposed a paper emergency-flatten runtime defect: paper market flatten orders filled but did not reduce synthetic wallet positions, causing retry loops until the container was stopped.
