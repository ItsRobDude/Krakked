...
class ExecutionService:
    ...
    def _validate_plan_freshness(
        ...
        """Check if the plan is too old to execute. Returns False if stale."""
    ...
    def _get_eligible_actions(self, plan: ExecutionPlan) -> List["RiskAdjustedAction"]:
        """Filter plan actions to find those that require execution."""
        eligible_actions = []
        for action in plan.actions:
            ...
            try:
                ...
                delta = float(delta_dec)  # noqa: F841
            except Exception:
    ...
    def execute_plan(self, plan: ExecutionPlan) -> ExecutionResult:
        ...
        * Records an in-memory execution result and registers each order for
        ...
        if not self._validate_plan_freshness(plan, result):
            return result
