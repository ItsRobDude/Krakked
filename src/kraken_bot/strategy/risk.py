...
class RiskEngine:
    ...
    def process_intents(
        ...
        """
        Processes a batch of strategy intents, applying risk limits and kill switches.

        This is the core risk evaluation pipeline. It transforms raw intents (what
        strategies want) into risk-adjusted actions (what the system allows),
        enforcing a strict precedence of safety rules.

        Risk Evaluation Pipeline:
        1.  **Context Construction**: Builds a snapshot of current equity, exposures,
            and portfolio drift using `build_risk_context`.
        2.  **Global Kill Switches**: Checks for manual overrides, portfolio drift,
            or max daily drawdown.
            *   If ACTIVE: Blocks all "open" and "increase" intents.
            *   Allows "reduce" or "exit" intents (unless they result in dust),
                enabling risk reduction even during emergencies.
        3.  **Aggregation**: Groups intents by pair to evaluate net exposure changes.
        4.  **Per-Pair Processing** (via `_process_pair_intents`):
            *   **Sizing**: Calculates target USD exposure (using volatility sizing
                if `desired_exposure_usd` is None).
            *   **Liquidity**: Caps exposure based on 24h volume (`min_liquidity_24h_usd`).
            *   **Limits**: Applies strategy caps, max per-asset exposure, and
                max portfolio leverage (`_apply_limits`).
            *   **Dust**: Rejects actions that would result in untradeable "dust"
                positions or tiny orders.

        Args:
            intents: List of raw intents generated by strategies.
            weights: Optional dynamic allocation weights (if enabled).
            pending_orders: Optional list of open orders to account for committed capital.

        Returns:
            A list of `RiskAdjustedAction` objects. Actions may be marked as
            `blocked=True` (fully rejected) or `clamped=True` (sized down).
        """
