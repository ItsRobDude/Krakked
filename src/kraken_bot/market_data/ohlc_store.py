...
class FileOHLCStore:
    ...
    def _df_to_bars(self, df: pd.DataFrame) -> List[OHLCBar]:
        ...
        Expects df index to be the timestamp.
        """
        timestamps = df.index.astype(int).values  # type: ignore
        opens = df["open"].values  # type: ignore
        highs = df["high"].values  # type: ignore
        lows = df["low"].values  # type: ignore
        closes = df["close"].values  # type: ignore
        volumes = df["volume"].values  # type: ignore
