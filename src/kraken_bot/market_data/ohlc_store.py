...
class FileOHLCStore:
    ...
    def _update_cache(self, pair: str, timeframe: str, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            timestamps = cast(Any, tail_df["timestamp"]).astype(int).tolist()
    ...
    def get_bars(self, pair: str, timeframe: str, lookback: int) -> List[OHLCBar]:
        ...
        with self._file_lock:
            ...
            try:
                ...
                # Optimization: bottlenecks showed df.to_dict('records') was slow
                timestamps = cast(Any, df["timestamp"]).astype(int).tolist()
