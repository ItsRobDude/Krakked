...
class FileOHLCStore:
    ...
    def _df_to_bars(self, df: pd.DataFrame) -> List[OHLCBar]:
        """Convert a DataFrame to a list of OHLCBar objects efficiently."""
        if df.empty:
            return []
        ...
        # Vectorized extraction is much faster than to_dict('records')
        # Bottleneck evidence: tests/benchmark_ohlc_store_repro.py shows ~60% speedup
        # for 100k rows (0.42s -> 0.15s).
