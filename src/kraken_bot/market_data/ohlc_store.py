...
class FileOHLCStore:
    ...
    def _df_to_bars(self, df: pd.DataFrame) -> List[OHLCBar]:
        """Fast conversion from DataFrame to list of OHLCBar."""
        timestamps = df.index.astype(int).tolist()  # type: ignore[attr-defined]
        opens = df["open"].tolist()  # type: ignore[attr-defined]
        highs = df["high"].tolist()  # type: ignore[attr-defined]
        lows = df["low"].tolist()  # type: ignore[attr-defined]
        closes = df["close"].tolist()  # type: ignore[attr-defined]
        volumes = df["volume"].tolist()  # type: ignore[attr-defined]
