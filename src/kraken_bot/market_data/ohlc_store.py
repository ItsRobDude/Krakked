...
class FileOHLCStore:
    ...
    def _df_to_bars(self, df: pd.DataFrame) -> List[OHLCBar]:
        """Optimized conversion from DataFrame to list of OHLCBar objects."""
        if df.empty:
            return []
        try:
            # Type ignores used because mypy struggles with pandas dynamic return types
            timestamps = df.index.astype(int).tolist()  # type: ignore
            opens = df["open"].tolist()  # type: ignore
            highs = df["high"].tolist()  # type: ignore
            lows = df["low"].tolist()  # type: ignore
            closes = df["close"].tolist()  # type: ignore
            volumes = df["volume"].tolist()  # type: ignore
            ...
            logger.error(f"Error converting DataFrame to bars: {e}")
            # Fallback to slower but robust method if column access fails
