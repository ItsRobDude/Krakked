        ...
        with self._file_lock:
            ...
            if key in self._bar_cache:
                ...
                if len(cached_bars) >= lookback:
                    # OHLCBar is frozen, safe to return direct references (slice creates new list)
                    return list(cached_bars[-lookback:])
            ...
            try:
                ...
                if success and lookback <= self._cache_size:
                    # OHLCBar is frozen, safe to return direct references
                    return list(self._bar_cache[key][-lookback:])
