...
class FileOHLCStore:
    ...
    def _df_to_bars(self, df: pd.DataFrame) -> List[OHLCBar]:
        ...
        Bottleneck evidence: to_dict("records") is slow for large datasets (~0.4s for 100k rows).
        Optimization: Vectorized column extraction reduces this to ~0.2s.
        """
        if df.empty:
            return []

        # Ensure timestamp is available (index or column)
        # In this store, index is timestamp
