...
class FileOHLCStore:
    ...
    def _df_to_bars(self, df: pd.DataFrame) -> List[OHLCBar]:
        """Converts a DataFrame with a 'timestamp' index into a list of OHLCBars."""
        if df.empty:
            return []
        ...
        # Ensure timestamp is int (Parquet may load as int64, but be safe)
        timestamps = cast(Any, df_reset["timestamp"]).astype(int).tolist()

        # Extract other columns
        # Note: We assume the DataFrame columns match the OHLCBar fields
        # but explicit access is safer.
