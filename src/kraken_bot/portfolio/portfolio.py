...
class Portfolio:
    ...
    def _round_vol(self, meta: Union[str, PairMetadata], vol: float) -> float:
        """Round volume to the pair's configured lot decimals using ROUND_FLOOR."""
        try:
            if isinstance(meta, str):
                meta = self.market_data.get_pair_metadata(meta)
    ...
    def _round_price(self, meta: Union[str, PairMetadata], price: float) -> float:
    ...
    def _process_trade(self, trade: Dict):
        ...
        # Bolt: Avoid duplicate get_pair_metadata API calls by caching the PairMetadata object and passing it directly to _round_vol/_round_price.
