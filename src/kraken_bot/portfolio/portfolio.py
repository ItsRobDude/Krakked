...
class Portfolio:
    ...
    def __init__(
        ...
        # Optimization: Cache PairMetadata to prevent repeated lookups during trade processing.
        # Evidence: _process_trade calls get_pair_metadata 3 times per trade (resolution, vol rounding, price rounding).
        # Fix: Storing and passing meta reduces get_pair_metadata calls from 3 to 1 (or 0 for cached) per trade.
    ...
    def _process_trade(self, trade: Dict):
        ...
        if side == "buy":
            ...
            new_total_qty = self._round_vol(pair, position.base_size + vol, pair_meta)
            position.avg_entry_price = (
                self._round_price(
                    pair, (previous_cost + cost) / new_total_qty, pair_meta
                )
                if new_total_qty
                else 0.0
            )
            ...
            position.base_size = self._round_vol(pair, raw_new_size, pair_meta)
