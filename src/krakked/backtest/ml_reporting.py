...
def validate_ml_walk_forward_report_payload(
    ...
    if not isinstance(summary.get("regression_calibration"), dict):
        raise ValueError(
    ...
    for index, fold in enumerate(summary.get("folds") or [], start=1):
        ...
        if not isinstance(fold["diagnostics"].get("features"), dict):
            raise ValueError(
                f"ML report fold {index} feature diagnostics are invalid in {resolved_path}"
            )
