...
def _pair_report(
    *,
    ...
    candidates = _instrument_candidates(pair, instruments, tickers_by_symbol)
...
def _instrument_candidates(
    ...
    for instrument in instruments:
        ...
        family = _contract_family(symbol)
        is_perpetual = symbol.startswith(("PI_", "PF_")) or str(
            instrument.get("type") or ""
        ).lower().endswith("perpetual")
        symbol_base, symbol_quote = _symbol_base_quote(symbol, quote_aliases)
        pair_matches = any(
...
def _candidate_sort_key(candidate: Mapping[str, Any]) -> tuple[int, str]:
