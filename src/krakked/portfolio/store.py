# src/krakked/portfolio/store.py
...
if TYPE_CHECKING:
    ...
    from krakked.strategy.models import DecisionRecord, ExecutionPlan
...
class PortfolioStore(abc.ABC):
    ...
    def save_ml_model_checkpoint(
        ...
        *,
        ...
        """Persist a crash-safe ML checkpoint when the backend supports it."""
    ...
    def load_ml_model_checkpoint(
        ...
        *,
        ...
        """Load a crash-safe ML checkpoint when the backend supports it."""
...
class SQLitePortfolioStore(PortfolioStore):
    ...
    def save_ml_model_checkpoint(
        ...
        *,
        ...
        """Persist a crash-safe ML checkpoint for resumable training state."""
        ...
        with self._lock:
            ...
            cursor.execute(
                """
                INSERT INTO ml_model_checkpoints (
                    strategy_id,
                    model_key,
                    checkpoint_kind,
                    label_type,
                ...
                ON CONFLICT(strategy_id, model_key, checkpoint_kind) DO UPDATE SET
                    label_type       = excluded.label_type,
                    framework        = excluded.framework,
                    version          = excluded.version,
                    updated_at       = excluded.updated_at,
    ...
    def load_ml_model_checkpoint(
        ...
        *,
        ...
        """Load a persisted ML checkpoint if present."""
        ...
        with self._lock:
            ...
            cursor.execute(
                """
                SELECT model_blob, updated_at, checkpoint_state, metadata_json
                FROM ml_model_checkpoints
                WHERE strategy_id = ?
                    AND model_key = ?
                    AND checkpoint_kind = ?
                """,
                (strategy_id, model_key, checkpoint_kind),
            )
            row = cursor.fetchone()
        ...
        try:
            ...
            if updated_at.tzinfo is None:
                updated_at = updated_at.replace(tzinfo=UTC)
            checkpoint_state = str(row[2] or "ready")
